...
Test::Result test_certstor_load_allcert()
   ...
   try
      ...
      result.confirm("Root cert found", store.find_cert(root_cert.subject_dn(), key_id) != Botan::nullopt);
      result.confirm("ValidCert found", store.find_cert(valid_cert.subject_dn(), key_id) != Botan::nullopt);
      return result;
      }
   catch(std::exception& e)
      {
