...
public:
   void tls_emit_data(Botan::span<const uint8_t>) override { m_result.test_failure("unsolicited call to tls_emit_data"); }
   void tls_record_received(uint64_t, Botan::span<const uint8_t>) override { m_result.test_failure("unsolicited call to tls_record_received"); }
         ...
         if(algo == "client_hello")
            ...
            try
               ...
               std::visit([&](auto ch) {
                  ...
                  if constexpr(std::is_same<Botan::TLS::Client_Hello_13, decltype(ch)>::value)
                     {
         ...
         if(algo == "server_hello")
            ...
            try
               ...
               std::visit([&](auto msg) {
                  ...
                  else if constexpr(std::is_same<Botan::TLS::Server_Hello_13, decltype(msg)>::value)
                     {
                     result.confirm("expected Server_Hello_13", msg_type == "server_hello_13");
                     }
                  else if constexpr(std::is_same<Botan::TLS::Hello_Retry_Request, decltype(msg)>::value)
                     {
