...
cSHAKE_XOF::cSHAKE_XOF(size_t capacity, Botan::span<const uint8_t> function_name) :
...
void cSHAKE_XOF::start_msg(Botan::span<const uint8_t> salt, Botan::span<const uint8_t> key) {
...
void cSHAKE_XOF::add_data(Botan::span<const uint8_t> input) {
   BOTAN_STATE_CHECK(!m_output_generated);
   m_keccak.absorb(input);
}

void cSHAKE_XOF::generate_bytes(Botan::span<uint8_t> output) {
