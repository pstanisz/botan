...
std::unique_ptr<Entropy_Source> Entropy_Source::create(Botan::string_view name)
   {
#if defined(BOTAN_HAS_SYSTEM_RNG)
   if(name == "system_rng")
      {
...
size_t Entropy_Sources::poll_just(RandomNumberGenerator& rng, Botan::string_view the_src)
   {
   for(auto& src : m_srcs)
      {
      if(src->name() == the_src)
