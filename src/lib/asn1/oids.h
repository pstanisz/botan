...
inline void add_oid(const OID& oid, Botan::string_view name)
   {
...
BOTAN_DEPRECATED("Use OID::register_oid")
BOTAN_UNSTABLE_API
void add_oid2str(const OID& oid, Botan::string_view name);

BOTAN_DEPRECATED("Use OID::register_oid")
BOTAN_UNSTABLE_API
void add_str2oid(const OID& oid, Botan::string_view name);
...
inline OID str2oid_or_empty(Botan::string_view name)
   {
...
BOTAN_DEPRECATED("Use OID::from_name")
inline OID lookup(Botan::string_view name)
   {
   return OID::from_name(name).value_or(OID());
   }
