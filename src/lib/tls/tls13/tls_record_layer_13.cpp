   ...
   public:
      TLSPlaintext_Header(std::vector<uint8_t> hdr, const bool check_tls13_version) {
         ...
         if(m_legacy_version.major_version() != 0x03) {
            throw TLS_Exception(AlertType::IllegalParameter, "Received unexpected record version");
         }
         ...
         if(m_fragment_length == 0 && type() != Record_Type::ApplicationData) {
            throw TLS_Exception(AlertType::DecodeError, "empty record received");
         }
         ...
         if(m_type == Record_Type::ApplicationData) {
            ...
            if(m_fragment_length > MAX_CIPHERTEXT_SIZE_TLS13) {
               throw TLS_Exception(AlertType::RecordOverflow, "Received an encrypted record that exceeds maximum size");
            }
         } else {
            // RFC 8446 5.1
...
Record_Layer::ReadResult<Record> Record_Layer::next_record(Cipher_State* cipher_state) {
   ...
   if(plaintext_header.type() == Record_Type::ChangeCipherSpec &&
      ...
      throw TLS_Exception(AlertType::UnexpectedMessage, "malformed change cipher spec record received");
   }
   ...
   if(record.type == Record_Type::ApplicationData) {
      if(cipher_state == nullptr) {
         ...
         throw TLS_Exception(AlertType::UnexpectedMessage, "premature Application Data received");
      }

      if(record.fragment.size() < cipher_state->minimum_decryption_input_length()) {
         throw TLS_Exception(AlertType::BadRecordMac, "incomplete record mac received");
      }
      ...
      if(record.type == Record_Type::ChangeCipherSpec) {
         ...
         throw TLS_Exception(AlertType::UnexpectedMessage, "protected change cipher spec received");
      }
