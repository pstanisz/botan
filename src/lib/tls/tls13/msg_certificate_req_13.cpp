...
Certificate_Request_13::Certificate_Request_13(const std::vector<uint8_t>& buf, const Connection_Side side)
   ...
   if(side != Connection_Side::Server)
      {
      throw TLS_Exception(AlertType::UnexpectedMessage, "Received a Certificate_Request message from a client");
      }
   ...
   if(!m_extensions.has<Signature_Algorithms>())
      {
      throw TLS_Exception(AlertType::MissingExtension, "Certificate_Request message did not provide a signature_algorithms extension");
      }
