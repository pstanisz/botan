...
void Server_Impl_13::handle(const Client_Hello_12& ch) {
   ...
   if(m_handshake_state.has_hello_retry_request()) {
      throw TLS_Exception(AlertType::UnexpectedMessage, "Received a TLS 1.2 Client Hello after Hello Retry Request");
   }
...
void Server_Impl_13::handle(const Client_Hello_13& client_hello) {
   ...
   if(is_initial_client_hello) {
      ...
      //   connections.
      if(exts.has<Cookie>()) {
         throw TLS_Exception(AlertType::IllegalParameter, "Received a Cookie in the initial client hello");
      }
   }
