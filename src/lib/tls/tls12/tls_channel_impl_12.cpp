   ...
   try {
      while(input_size) {
         ...
         if(m_record_buf.size() > MAX_PLAINTEXT_SIZE) {
            throw TLS_Exception(AlertType::RecordOverflow, "TLS plaintext record is larger than allowed maximum");
         }
         ...
         if(record.type() != Record_Type::Alert) {
            if(initial_record) {
               ...
               if(record.version().major_version() != 3 && record.version().major_version() != 0xFE) {
                  throw TLS_Exception(AlertType::ProtocolVersion, "Received unexpected record version in initial record");
               }
            } else if(auto pending = pending_state()) {
               if(pending->server_hello() != nullptr && record.version() != pending->version()) {
                  throw TLS_Exception(AlertType::ProtocolVersion, "Received unexpected record version");
               }
...
void Channel_Impl_12::secure_renegotiation_check(const Client_Hello_12* client_hello) {
   ...
   if(auto active = active_state()) {
      ...
      if(active_sr != secure_renegotiation) {
         throw TLS_Exception(AlertType::HandshakeFailure, "Client changed its mind about secure renegotiation");
      }
   }
   ...
   if(secure_renegotiation) {
      ...
      if(data != secure_renegotiation_data_for_client_hello()) {
         throw TLS_Exception(AlertType::HandshakeFailure, "Client sent bad values for secure renegotiation");
      }
   }
}
...
void Channel_Impl_12::secure_renegotiation_check(const Server_Hello_12* server_hello) {
   ...
   if(auto active = active_state()) {
      ...
      if(active_sr != secure_renegotiation) {
         throw TLS_Exception(AlertType::HandshakeFailure, "Server changed its mind about secure renegotiation");
      }
   }
   ...
   if(secure_renegotiation) {
      ...
      if(data != secure_renegotiation_data_for_server_hello()) {
         throw TLS_Exception(AlertType::HandshakeFailure, "Server sent bad values for secure renegotiation");
      }
   }
}
