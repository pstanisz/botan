...
StreamCipher_Filter::StreamCipher_Filter(Botan::string_view sc_name) :
   m_cipher(StreamCipher::create_or_throw(sc_name)),
   m_buffer(m_cipher->buffer_size())
   {
   }

StreamCipher_Filter::StreamCipher_Filter(Botan::string_view sc_name, const SymmetricKey& key) :
...
Hash_Filter::Hash_Filter(Botan::string_view hash_name, size_t len) :
...
MAC_Filter::MAC_Filter(Botan::string_view mac_name, size_t len) :
   m_mac(MessageAuthenticationCode::create_or_throw(mac_name)),
   m_out_len(len)
   {
   }

MAC_Filter::MAC_Filter(Botan::string_view mac_name, const SymmetricKey& key, size_t len) :
