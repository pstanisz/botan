...
using convertible_to =
   typename std::void_t<decltype(std::is_convertible_v<FromT, ToT>), decltype(static_cast<ToT>(std::declval<FromT>()))>;
...
using equality_comparable =
   typename std::enable_if_t<is_convertible_to_v<decltype(std::declval<const std::remove_reference_t<T>&>() ==
...
template <typename T>
using less_comparable =
   typename std::enable_if_t<is_convertible_to_v<decltype(std::declval<const std::remove_reference_t<T>&>() <
                                                          std::declval<const std::remove_reference_t<T>>()),
                                                 bool>>;

template <typename T>
using greater_comparable =
   typename std::enable_if_t<is_convertible_to_v<decltype(std::declval<const std::remove_reference_t<T>&>() >
                                                          std::declval<const std::remove_reference_t<T>>()),
                                                 bool>>;
...
template <typename T>
using default_initializable = typename std::
   void_t<std::enable_if_t<is_constructible_from_v<T>>, decltype(T{}), decltype(::new(static_cast<void*>(nullptr)) T)>;
...
template <typename IterT, typename ContainerT>
using container_iterator = typename std::enable_if_t<same_as_v<IterT, typename ContainerT::iterator> ||
                                                     same_as_v<IterT, typename ContainerT::const_iterator>>;
...
template <typename PtrT, typename ContainerT>
using container_pointer = typename std::enable_if_t<same_as_v<PtrT, typename ContainerT::pointer> ||
                                                    same_as_v<PtrT, typename ContainerT::const_pointer>>;
...
using contiguous_container =
   typename std::enable_if_t<is_container_v<T> && is_container_pointer_v<decltype(std::declval<T&>().data()), T>>;
...
template <typename T>
using resizable_container = typename std::void_t<std::enable_if_t<is_container_v<T>>,
...
template <typename T>
using resizable_byte_buffer = typename std::enable_if_t<is_contiguous_container_v<T> && is_resizable_container_v<T> &&
...
template <typename T>
using contiguous_strong_type = typename std::enable_if_t<is_strong_type_v<T> && is_contiguous_container_v<T>>;

template <typename T, typename = void>
struct is_contiguous_strong_type : std::false_type {};

template <typename T>
struct is_contiguous_strong_type<T, contiguous_strong_type<T>> : std::true_type {};

template <typename T>
constexpr bool is_contiguous_strong_type_v = is_contiguous_strong_type<T>::value;

template <typename T>
constexpr bool is_integral_v = std::is_integral_v<T>;

template <typename T1, typename T2>
constexpr bool is_both_integral_v = std::is_integral_v<T1> && std::is_integral_v<T2>;
