...
class BOTAN_PUBLIC_API(2,0) SQL_Database
   ...
   public:
      ...
      class BOTAN_PUBLIC_API(2,0) SQL_DB_Error final : public Exception
         {
         public:
            explicit SQL_DB_Error(Botan::string_view what) :
               Exception("SQL database", what),
               m_rc(0)
               {}

            SQL_DB_Error(Botan::string_view what, int rc) :
               Exception("SQL database", what),
               m_rc(rc)
               {}
      ...
      class BOTAN_PUBLIC_API(2,0) Statement
         ...
         public:
            ...
            virtual void bind(int column, Botan::string_view str) = 0;
      ...
      */
      virtual std::shared_ptr<Statement> new_statement(Botan::string_view base_sql) const = 0;

      virtual size_t row_count(Botan::string_view table_name) = 0;

      virtual void create_table(Botan::string_view table_schema) = 0;

      virtual size_t rows_changed_by_last_statement() = 0;

      virtual size_t exec(Botan::string_view sql) { return new_statement(sql)->spin(); }
