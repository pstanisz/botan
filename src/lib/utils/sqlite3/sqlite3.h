...
class BOTAN_PUBLIC_API(2,0) Sqlite3_Database final : public SQL_Database
   ...
   public:
       ...
       */
      Sqlite3_Database(Botan::string_view file, Botan::optional<int> sqlite_open_flags = Botan::nullopt);

      ~Sqlite3_Database();

      size_t row_count(Botan::string_view table_name) override;

      void create_table(Botan::string_view table_schema) override;

      size_t rows_changed_by_last_statement() override;

      std::shared_ptr<Statement> new_statement(Botan::string_view sql) const override;
   ...
   private:
      class Sqlite3_Statement final : public Statement
         {
         public:
            void bind(int column, Botan::string_view val) override;
            ...
            Sqlite3_Statement(sqlite3* db, Botan::string_view base_sql);
            ~Sqlite3_Statement();
         private:
            sqlite3_stmt* m_stmt;
         };
