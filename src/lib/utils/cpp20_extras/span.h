/*
* Implementation of C++20 Botan::span replacement for compilers not supporting C++20
...
struct is_container {
      static constexpr bool value =
         !is_span_v<U> && !is_std_array_v<U> && !std::is_array_v<U> && has_size_v<C> && has_data_v<C>;
};
...
struct is_container_type_compatible<
   C,
   U,
   typename std::enable_if_t<
      std::is_convertible_v<std::remove_pointer_t<decltype(Botan::detail::data(std::declval<C>()))> (*)[], U (*)[]>>>
      : std::true_type {};
   ...
   public:
      ...
      template <typename Iter_first,
                typename std::enable_if_t<detail::is_dynamic_extent_v<Extent> &&
                                             detail::is_span_convertible_v<Iter_first, Type>,
                                          bool> = true>
      ...
      template <std::size_t N, typename std::enable_if_t<detail::is_span_capacity_v<Extent, N>, bool> = true>
      ...
      template <typename Container,
                ...
                typename std::enable_if_t<detail::is_dynamic_extent_v<E> && detail::is_container_v<Container> &&
                                             detail::is_container_type_compatible_v<Container&, Type>,
                                          bool> = true>
      constexpr span(Container& cont) : m_data(Botan::detail::data(cont)), m_size(Botan::detail::size(cont)) {}
      ...
      template <typename Container,
                ...
                typename std::enable_if_t<detail::is_dynamic_extent_v<E> && detail::is_container_v<Container> &&
                                             detail::is_container_type_compatible_v<Container&, Type>,
                                          bool> = true>
      constexpr span(const Container& cont) : m_data(Botan::detail::data(cont)), m_size(Botan::detail::size(cont)) {}
      ...
      template <typename U,
                ...
                typename std::enable_if_t<!detail::is_span_explicit_v<Extent, N> &&
                                             detail::is_span_copyable_v<Type, Extent, U, N>,
                                          bool> = true>
      ...
      constexpr span<Type, dynamic_extent> first(size_type count) const {
      ...
      constexpr span<Type, dynamic_extent> last(size_type count) const {
         return span<Type, dynamic_extent>(data() + (size() - count), count);
      }
...
template <typename Type, std::size_t Extent>
span<const std::byte, (detail::is_dynamic_extent_v<Extent> ? dynamic_extent : sizeof(Type) * Extent)> as_bytes(
   span<Type, Extent> s) noexcept {
   return {reinterpret_cast<const std::byte*>(s.data()), s.size_bytes()};
}

template <typename Type, size_t Extent, typename std::enable_if_t<!std::is_const_v<Type>, bool> = true>
span<std::byte, (detail::is_dynamic_extent_v<Extent> ? dynamic_extent : sizeof(Type) * Extent)> as_writable_bytes(
   span<Type, Extent> s) noexcept {
