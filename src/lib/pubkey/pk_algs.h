...
std::unique_ptr<Public_Key> load_public_key(const AlgorithmIdentifier& alg_id, Botan::span<const uint8_t> key_bits);

BOTAN_PUBLIC_API(2, 0)
std::unique_ptr<Private_Key> load_private_key(const AlgorithmIdentifier& alg_id, Botan::span<const uint8_t> key_bits);
