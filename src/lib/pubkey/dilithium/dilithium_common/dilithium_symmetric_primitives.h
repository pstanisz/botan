...
class Dilithium_Symmetric_Primitives
   ...
   public:
      ...
      secure_vector<uint8_t> ExpandMask(Botan::span<const uint8_t> seed,
      ...
      virtual std::unique_ptr<StreamCipher> XOF(const XofType type, Botan::span<const uint8_t> seed,
...
class DilithiumModeConstants
   ...
   public:
      ...
      std::unique_ptr<StreamCipher> XOF_128(Botan::span<const uint8_t> seed,
                                            uint16_t nonce) const
         {
         return this->m_symmetric_primitives->XOF(Dilithium_Symmetric_Primitives::XofType::k128, seed, nonce);
         }
      std::unique_ptr<StreamCipher> XOF_256(Botan::span<const uint8_t> seed,
                                            uint16_t nonce) const
         {
