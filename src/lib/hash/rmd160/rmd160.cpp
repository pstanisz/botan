...
*/
void RIPEMD_160::compress_n(digest_type& digest, Botan::span<const uint8_t> input, size_t blocks) {
...
void RIPEMD_160::add_data(Botan::span<const uint8_t> input) {
   m_md.update(input);
}

void RIPEMD_160::final_result(Botan::span<uint8_t> output) {
   m_md.final(output);
}
