...
void SHAKE_128::add_data(Botan::span<const uint8_t> input) {
   m_keccak.absorb(input);
}

void SHAKE_128::final_result(Botan::span<uint8_t> output) {
   m_keccak.finish();
...
void SHAKE_256::add_data(Botan::span<const uint8_t> input) {
   m_keccak.absorb(input);
}

void SHAKE_256::final_result(Botan::span<uint8_t> output) {
   m_keccak.finish();
   m_keccak.squeeze(output);
   clear();
}
