...
class BOTAN_PUBLIC_API(2, 0) RandomNumberGenerator {
   public:
      ...
      */
      void randomize(Botan::span<uint8_t> output) { this->fill_bytes_with_input(output, {}); }

      void randomize(uint8_t output[], size_t length) { this->randomize(Botan::span(output, length)); }
      ...
      */
      ...
      void add_entropy(const uint8_t input[], size_t length) { this->add_entropy(Botan::span(input, length)); }
      ...
      */
      void random_vec(Botan::span<uint8_t> v) { this->randomize(v); }
      ...
      */
      template<typename T = secure_vector<uint8_t>, 
         typename = std::enable_if_t<concepts::is_resizable_byte_buffer_v<T> && concepts::is_default_initializable_v<T>>>
